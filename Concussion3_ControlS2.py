...
file_run1 = os.path.join(base_dir, "data", "Control_4.1.counts.tsv")
file_run2 = os.path.join(base_dir, "data", "Control_4.2.counts.tsv")
file_run3 = os.path.join(base_dir, "data", "Control_4.3.counts.tsv")
file_run4 = os.path.join(base_dir, "data", "Control_6.1.counts.tsv")
file_run5 = os.path.join(base_dir, "data", "Control_6.2.counts.tsv")
file_run6 = os.path.join(base_dir, "data", "Control_6.3.counts.tsv")
file_run7 = os.path.join(base_dir, "data", "Control_7.1.counts.tsv")
file_run8 = os.path.join(base_dir, "data", "Control_7.2.counts.tsv")
file_run9 = os.path.join(base_dir, "data", "Control_7.3.counts.tsv")
...
df_run8 = pd.read_csv(file_run8, sep='\t')
df_run9 = pd.read_csv(file_run9, sep='\t')
...
df_run8.set_index("barcode", inplace=True)
df_run9.set_index("barcode", inplace=True)
...
df_combined = df_run1.add(df_run2, fill_value=0).add(df_run3, fill_value=0).add(df_run4, fill_value=0).add(df_run5, fill_value=0).add(df_run6, fill_value=0).add(df_run7, fill_value=0).add(df_run8, fill_value=0).add(df_run9, fill_value=0)
# Bring back 'barcode' back as normal column instead of the index:
df_combined.reset_index(inplace=True)
# Save the combined results to a new TSV file
control_5 = os.path.join(base_dir, "combined_runs", "combined_runs_ControlS2.tsv")
