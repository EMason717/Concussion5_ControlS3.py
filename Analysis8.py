...
#switch rows to columns and columns to rows, so it analyzes per gene rather than per barcode
control_df = newcontroldf.T
cte_df = newctedf.T
...
def main():
    ...
    base_dir = os.path.join(os.getcwd(), 'combined_runs')
    control_path = os.path.join(base_dir, 'combined_runs_ControlS8.tsv')
    cte_path = os.path.join(base_dir, 'combined_runs_CTES8.tsv')
    ...
    result_path = os.path.join(output_dir, 'analysisS8.tsv')
